...
const ArticleForm = ({ onSubmit, articleToEdit, workspaceId, onCreated }) => {
    ...
    const handleFileChange = (e) => {
        ...
        if (invalidFiles.length > 0) {
            setWarning(`!Внимание: выбранные файлы не будут прикреплены: ${invalidFiles.map(f => f.name).join(", ")}. Допустимые форматы: ${allowedExtensions.join(', ')}`);
        } else {
            setWarning("");
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!articleToEdit && onCreated) onCreated();
